...
if(NOT DEFINED KATESTROPHE_INCLUDED)
  ...
  # sets the provided output variable KATESTROPHE_OVERSUBSCRIBE_FLAG to the
  # flags required to run mpiexec with more MPI ranks than cores available
  ...
  add_library(
    mpi-gtest-main EXCLUDE_FROM_ALL
    ${CMAKE_CURRENT_LIST_DIR}/mpi_gtest_main.cpp
  )
  ...
  # executed with ctest
  #
  # KATESTROPHE_TEST_TARGET target name DISCOVER_TESTS sets whether the
  # individual tests should be added to the ctest output (like
  # gtest_discover_tests) CORES the number of MPI ranks to run the test with
  #
    ...
    foreach(p ${KATESTROPHE_CORES})
      ...
      if(KATESTROPHE_DISCOVER_TESTS)
        ...
        katestrophe_discover_tests(
          ...
          MPI_EXEC_COMMAND
          "${MPI_EXEC_COMMAND}"
          # PROPERTIES ENVIRONMENT "ASAN_OPTIONS=detect_leaks=0" # Prevent
          # memory leaks in OpenMPI from # making the test fail.
        )
      else()
        add_test(
          NAME "${TEST_NAME}"
